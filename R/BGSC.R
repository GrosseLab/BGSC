...
#' @author Claus Weinholdt
#' @param set define which data set is used; defalut 'SF767'
#' @param expressionOut if TRUE return of log2 exprssion 
#' @return a \code{data.table} 
#' @export
getQPCR <- function( set = 'SF767',expressionOut=FALSE){   
  ### load data ---------------------------
  print(paste0("Dataset is ",set))
  if (set == 'SF767') {
    ...
    data(RawQPCR799, envir = environment()) 
    RawQPCRsf <- RawQPCR799
    print(RawQPCRsf)
    
  }else{
    stop("error: set not defined. Have to be 'SF767' or 'L799' ", call. = FALSE)
  }
  ...
  relative.Werte.Mean <- t(do.call(cbind,lapply(relative.Werte, function(mat) rowMeans((mat)))))
  colnames(relative.Werte.Mean) <- c(1:6)
  # print(log2(relative.Werte.Mean)[,2] - log2(relative.Werte.Mean)[,1] )
  ...
  # print(relative.Werte.geoMean[,2] - (relative.Werte.geoMean)[,1] ) 
  relative.Werte.geoMean.stderr <- do.call(rbind,lapply(relative.Werte, function(mat) apply(mat,1,function(row) plotrix::std.error(log2(row),na.rm = T) ) ))
  colnames(relative.Werte.geoMean.stderr) <- c(1:6)
  
  if(expressionOut==TRUE){
    return(list("log2geoMean"=relative.Werte.geoMean,log2geoStdErr=relative.Werte.geoMean.stderr))
  }
  ...
  delta.s1stderr <- apply(delta.ct.Mean[,c1],MARGIN = 1, function(row) plotrix::std.error(row,na.rm = T) )
  delta.s0stderr <- apply(delta.ct.Mean[,c0],MARGIN = 1, function(row) plotrix::std.error(row,na.rm = T) )
  delta.s0s1stderr = sqrt( delta.s1stderr^2 +  delta.s0stderr^2)
  ...
  ### return---------------------------
  qCPRdataC <- data.table("rn"=names(relative.Werte) ,relative.Werte.geoMean,relative.Werte.geoMean.C1,relative.Werte.geoMean.C0,relative.Werte.geoMean.C1C0.logFC,
